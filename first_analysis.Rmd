...
mean_list <- seq(0.1,2.5,length.out=100)
...
mpt_fun <- function(x) {
  bvec_m <- c(x, 1,rep(0, 21))
  y = solve.QP(dmat, dvec_matrix, amat_matrix, bvec_m, meq = 2)
...
rownames(solution) <- 1:nrow(solution) #rename rows
...
for(i in 1:n){
  ...
  rand[i,nvar+2]<-mu_fcn(rand[i,1:nvar],pop_return_mean_df_t) #do we need to use the transposed version
  
} ##getting an error in here "can't run .temp" - idk what temp is?
...
```{r}
hold=217#Chosen to best show case results


p1<-ggplot(data=var_out,aes(y=port_mean,x=var_res))+
...
```{r}
p_2019<-ggplot(data=var_out,aes(y=port_mean,x=var_res))+
  #geom_point(color="grey",alpha=0.5)+
  #geom_point(aes(x=rand[hold,4],y=rand[hold,5]),size=3)+
  #geom_line()+
  geom_line() +
  geom_point(aes(x=6.889624, y=0.7743007)) +
  #annotate("label",x=.8,y=1.75,label=paste("Weights currently:\n","(",paste(round(rand[hold,1:3],3),collapse = ", "),")"))+
  #xlim(-0.5,1.5)+
  #ylim(0,3.5)+
  theme_classic()
...
```{r}
p_test<-ggplot(data=var_out,aes(y=port_mean,x=var_res))+
  #geom_point(color="grey",alpha=0.5)+
  #geom_point(aes(x=rand[hold,4],y=rand[hold,5]),size=3)+
  #geom_line()+
  geom_line() +
  #geom_point(aes(x=1.14400840, y=1.39769)) + 2017
  geom_point(aes(x=0.7743007, y=0.5501547)) + #mean of all reutrns (what we first ran QuadProg on)
  annotate("label",x=0.77,y=0.55,label=paste("mean of mean returns weights x=0.774 y=0.550"), hjust=-.05)+
  geom_point(aes(x=0.21176523, y=0.1335914)) + # 2018 returns mpt_fun(0.21176523) = var*2
  annotate("label",x=0.21,y=0.133,label=paste("2018 Weights currently x=0.21, y=0.133"), hjust=-.05)+#:\n","(",paste(round(rand[hold,1:21],3),collapse = ", "),")"), hjust=-0.1)+
  #annotate("label",x=.8,y=1.75,label=paste("Weights currently:\n","(",paste(round(rand[hold,1:3],3),collapse = ", "),")"))+
  #xlim(-0.5,1.5)+
  #ylim(0,3.5)+
  theme_classic()
...
```{r}
var_keep=which.min(abs(var_out$var_res-rand[hold,4]))
mean_keep=which.min(abs(mu_list-rand[hold,5]))

p2<-p1+ annotate("segment",x=rand[hold,4],y=rand[hold,5],xend=var_out$var_res[var_keep],yend=mu_list[var_keep],color="red")+
  annotate("pointrange",x=var_out$var_res[var_keep],y=mu_list[var_keep],ymin=rand[hold,5],ymax=mu_list[mean_keep],color="red")+
  annotate("label",x=0.35,y=2.8,label=paste("Mean maximizing weights:\n","(",paste(round(front$solution[[var_keep]],3),collapse=", "),")"),color="red")
...
#Copy of initial portfolio theory
```{r}
## first row of amat
returns_2019_df <- c(0.05301915, -0.38815789, -0.446385380, -0.51538230, -0.29841897, 0.55219512, 0.26380898, -0.432225064, -0.658591102, -0.7650369, -0.1011357, -0.5186722, -0.09069307, 1.13663034, 0.13830272, 0.13218391, -0.12425595, 0.231893266, -0.4369491, -0.481514878, 0.36549405) # making  df with just the mean returns from each population
colnames(returns_2019_df) <- c('1':'21') # renaming column headers
...
amat<- rbind(returns_2019_df, line_2_df, id_matrix)
...
```{r}
rowMeans(returns) #year 2019 = -0.11351862 ((mean return of all 21 assets in 2019)

mpt_fun(1.1)
mpt_fun(-0.1) #won't plot becuase below threshold of means that we set
mpt_fun(0.21176523) #var = 0.1335914 2018 returns
mpt_fun(1.14400840)
```
