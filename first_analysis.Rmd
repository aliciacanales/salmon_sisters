...
weights_table_2019 <- weights_2019 %>% 
  round(3) %>% 
  kable() %>% 
  kable_styling(bootstrap_options = "striped",
                position = "center", full_width = FALSE)

weights_table_2019 # weights for 2019 in a kable


weights_mean_df<- as.data.frame(var_out$weights[var_keep]) 

mean_2019_eff_f <- weights_mean_df %>%
  round(3) %>% 
  kable() %>% 
  kable_styling(bootstrap_options = "striped",
                position = "center", full_width = FALSE)
mean_2019_eff_f ## weights for 2019 when variance is held


weights_df<- as.data.frame(var_out$weights[mean_keep])
