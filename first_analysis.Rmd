...
p1<-ggplot(as.data.frame(rand),aes(x=var,y=mu))+
  geom_point(color="grey",alpha=0.5)+
  geom_point(aes(x=2.656514,y=0.7697386),size=3)+
...
p_a<-ggplot(data=var_out,aes(y=port_mean,x=var_res))+
  ...
  #geom_line()+
  geom_line() +
  geom_point(aes(x=2.656514, y=0.7697386)) +
  annotate("label",x=17,y=.7,label=paste("Weights currently:\n","(",paste(round(rand[1,1:10],2),collapse = ", "),")"))+
  #xlim(-0.5,1.5)+
  #ylim(0,3.5)+
  theme_classic()
...
mean_keep=which.min(abs(mean_list-rand[hold,5]))

p2 <- p1 + annotate("segment",x=2.657,y=0.7697,xend=var_out$var_res[var_keep],yend=mean_list[var_keep],color="red")+
