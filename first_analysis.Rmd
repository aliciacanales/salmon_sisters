...
var_out<-solution %>% 
  ...
  mutate(var_res=map_dbl(.x=weights,~var_df_fcn(.x,dmat)))
...
for(i in 1:n){
  ...
  rand[i,nvar+2]<-mu_fcn(rand[i,1:nvar],pop_return_mean_df)
  
} ##getting an error in here "can't run .temp" - idk what temp is?

colnames(rand)<-c(paste0("weight_",1:length(pop_return_mean_df)),"var","mu")
```
...
p1<-ggplot(as.data.frame(rand),aes(x=var,y=mu))+
  ...
  geom_line(data=var_out,aes(y=mu_list,x=var_res))+
